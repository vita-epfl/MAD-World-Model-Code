...
class MediaDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        if str(video_path).__contains__("pose"):
            video_path = Path(str(video_path).replace("pose", f"bbox_{rand_id}"))
            # video_path = Path(str(video_path).replace("pose", f"hdmap"))
            # print("have replaced pose with bbox", video_path)

        # Compute relative path of the video
        data_root = self.dataset_file.parent
        relative_path = str(video_path.relative_to(data_root))
        media_relative_path = str(self.main_media_paths[index].relative_to(data_root)).replace("rgb", f"bbox_{rand_id}")
    ...
    def _load_video_paths_from_json(self, column: str) -> list[Path]:
        ...
        for path in video_paths:
            if str(path).__contains__("pose"):
                for rnd_id in range(5):
                    alt_path = Path(str(path).replace("pose", f"bbox_{rnd_id}"))
                    # alt_path = Path(str(path).replace("pose", "hdmap"))
        ...
        # if invalid_paths:
        #     raise ValueError(f"Found {len(invalid_paths)} invalid video paths. First few: {invalid_paths[:5]}")
        # remove invalid paths from video_paths
        if invalid_paths:
            logger.warning(f"Found {len(invalid_paths)} invalid video paths. First few: {invalid_paths[:5]}")
            video_paths = [path for path in video_paths if path not in invalid_paths]
...
def compute_vae_patch_masks_simple(
    ...
    for s in range(1, F_, VAE_TEMPORAL_FACTOR):
...
def compute_video_latents_mask(
        dataset_file: str | Path,
        video_column: str,
        resolution_buckets: list[tuple[int, int, int]],
        output_dir: str,
    ...
    """
    Process videos and save a mask of where in the latents are interesting.
    ...
    with Progress(
        ...
        for batch in dataloader:
            ...
            with torch.inference_mode():
                patch_masks_batch = []
                for vd in batch["video"]:
                    patch_masks, _ = compute_vae_patch_masks_simple((vd + 1) / 2, min_active_frac=0.05)
                    patch_masks = patch_masks.flatten()
                    patch_masks_batch.append(patch_masks)
            ...
            for i in range(len(batch["relative_path"])):
                ...
                mask_data = {
                    "patch_masks": patch_masks_batch[i].cpu().contiguous(),
    ...
    with Progress(
        ...
        for batch in dataloader:
            ...
            with torch.inference_mode():
                vd = batch["video"]
                if vd.shape[1] == 61:
                    new_vd = torch.zeros(vd.shape[0], 121, vd.shape[2], vd.shape[3], vd.shape[4], dtype=vd.dtype, device=vd.device)
                    new_vd[:, ::2] = vd
                    new_vd[:, 1::2] = vd[:, 1:]
                    vd = new_vd
                    for i in range(len(batch["video_metadata"]["fps"])):
                        batch["video_metadata"]["fps"][i] = batch["video_metadata"]["fps"][i] * 2
                elif vd.shape[1] == 59:
                    # we put the first frame as white, then every two frames are the same, and will just repeat the last frame twice more
                    new_vd = torch.zeros(vd.shape[0], 121, vd.shape[2], vd.shape[3], vd.shape[4], dtype=vd.dtype, device=vd.device)
                    new_vd[:, 1:-2:2] = vd
                    new_vd[:, 2:-2:2] = vd
                    new_vd[:, 0] = 1.0  # white frame
                    new_vd[:, -2:] = vd[:, -1:].repeat(1, 2, 1, 1, 1)
                    vd = new_vd
                    for i in range(len(batch["video_metadata"]["fps"])):
                        batch["video_metadata"]["fps"][i] = batch["video_metadata"]["fps"][i] * 2
                # torch.all(vd[0, 3] == vd[0, 4])
                # vd[0, 1::2] = -1 # make every second frame black for testing
                # vd[0, 1:120:2] = vd[0, 0:120:2]  # make every second frame same as previous for testing
                # vd = vd[:, ::2][:, :57]  # downsample to 12 fps for faster processing
                video_latents = encode_video(vae=vae, image_or_video=vd, device=device,)
