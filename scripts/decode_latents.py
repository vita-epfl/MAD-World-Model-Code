...
class LatentsDecoder:
    ...
    def decode(self, latents_dir: Path, output_dir: Path, seed: int | None = None, num_videos: int = -1) -> None:
        ...
        if num_videos > 0:
            latent_files = latent_files[:num_videos]
            logger.info(f"Decoding only the first {num_videos} files as specified")
    ...
    def _process_file(self, latent_file: Path, output_dir: Path, seed: int | None) -> None:
        """Process a single latent file (PNG for single frame, MP4 via imageio-ffmpeg for video)."""
        # Load latent
        data = torch.load(latent_file, map_location=self.device)

        # (Optional) debug dump
        print("DEBUG contents of data:")
        for k, v in data.items():
            if torch.is_tensor(v):
                print(f"  {k}: torch.Tensor shape={tuple(v.shape)} dtype={v.dtype}")
            else:
                print(f"  {k}: {type(v)} = {v}")
        ...
        # ---- Decode ----
        print("DEBUG start decoding")
        # Be strict about ints here
        T = int(data["num_frames"])
        H_lat = int(data["height"])
        W_lat = int(data["width"])
        ...
        # Remove batch dim -> [C,F,H,W], convert to uint8 and permute to [F,H,W,C]
        video = video[0]
        video = (video * 255).round().clamp(0, 255).to(torch.uint8)
        video = video.permute(1, 2, 3, 0).contiguous()

        F, H, W, C = [int(x) for x in video.shape]
        print("DEBUG start saving", "shape=", (F, H, W, C), "dtype=", video.dtype, "device=", video.device)

        # Single-frame -> PNG
        if F == 1:
            output_path = output_dir / f"{latent_file.stem}.png"
            torchvision.utils.save_image(
                video[0].permute(2, 0, 1).float() / 255.0,  # [H,W,C] -> [C,H,W], float in [0,1]
                str(output_path),
            )
            print("DEBUG finish saving (PNG)")
            return

        # Multi-frame -> MP4 via imageio-ffmpeg
        output_path = output_dir / f"{latent_file.stem}.mp4"

        # Normalize FPS to a plain Python float
        fps_val = data.get("fps", 24)
        if torch.is_tensor(fps_val):
            fps_py = float(fps_val.item())
        elif isinstance(fps_val, np.generic):
            fps_py = float(fps_val.item())
        else:
            fps_py = float(fps_val)

        # Move to CPU numpy (imageio expects ndarray)
        vid_np = video.cpu().numpy()  # (F,H,W,C) uint8

        # Write with imageio (libx264), avoiding 16x macroblock resizing
        try:
            writer = imageio.get_writer(
                ...
                codec="libx264",
                pixelformat="yuv420p",
                macro_block_size=None,  # don’t pad to multiples of 16
                quality=8,              # or set bitrate=...
            )
            for f in range(F):
                writer.append_data(vid_np[f])  # (H,W,C) uint8
            writer.close()
            print("DEBUG finish saving (MP4)")
        except Exception as e:
            # Helpful debug
            print("DEBUG save failed with:", repr(e))
            print("DEBUG types:", "fps:", type(fps_py), fps_py,
                "shape:", (F, H, W, C), "dtype:", vid_np.dtype)
            raise
