...
class LTXConditionPipeline(DiffusionPipeline, FromSingleFileMixin, LTXVideoLoraLoaderMixin):
    ...
    @torch.no_grad()
    def compute_vae_patch_masks_simple(
            self,
            ref_frames: torch.Tensor,  # (F, C, H, W)
            black_thresh: float = 1e-6,  # pixel active if abs(.) > this in any channel
            min_active_frac: float = 0.0,  # e.g., 0.05 -> require ≥5% active pixels in a patch
    ) -> Dict[str, object]:
        """
            'patch_masks': BoolTensor [G, H//32, W//32],  # per temporal-group patch mask
            'kept_patch_indices': List[Tensor],           # per group, 1D indices of True patches
        """
        assert ref_frames.dim() == 4, "ref_frames must be (F, C, H, W)"
        F_, C, H, W = ref_frames.shape
        S = self.vae_spatial_compression_ratio
        assert H % S == 0 and W % S == 0, "H and W must be divisible by 32"

        # Per-pixel activity across channels: (F, H, W) bool
        active = (ref_frames.abs().amax(dim=1) > black_thresh)

        # Temporal groups: first frame alone, then groups of 8
        group_ranges: List[Tuple[int, int]] = []
        if F_ >= 1:
            group_ranges.append((0, 1))
        for s in range(1, F_, self.vae_temporal_compression_ratio):
            group_ranges.append((s, min(s + 8, F_)))

        masks = []
        kept_indices = []

        # Helper: reduce (H,W) bool -> (H//32, W//32) bool via pooling
        def to_patch_mask(bmap_hw: torch.Tensor) -> torch.Tensor:
            x = bmap_hw.float().unsqueeze(0).unsqueeze(0)  # (1,1,H,W)
            frac = F.avg_pool2d(x, kernel_size=S, stride=S)  # (1,1,H//32,W//32)
            pmask = (frac >= min_active_frac) if min_active_frac > 0.0 else (frac > 0.0)
            return pmask[0, 0].bool()  # (H//32, W//32)

        for (s, e) in group_ranges:
            # Any frame in the group activates the spatial location
            g_active_hw = active[s:e].any(dim=0)  # (H, W) bool
            pmask = to_patch_mask(g_active_hw)  # (H//32, W//32) bool
            masks.append(pmask)
            kept_indices.append(pmask.flatten().nonzero(as_tuple=False).squeeze(1))

        patch_masks = torch.stack(masks, dim=0)  # (G, H//32, W//32)

        return patch_masks, kept_indices
    ...
    def __call__(
        ...
        reference_video_fps: Optional[int] = 24,
        use_reference_video_as_noise: bool = False,
        filter_reference_video_patches: bool = False,
        ...
        Args:
            ...
            reference_image (`PipelineImageInput` or `List[PipelineImageInput]`, *optional*):
                An optional reference image or list of images to guide the generation process. If a list of images
                is provided, its length must match the batch size. The reference image(s) will be encoded and
                concatenated to the latent sequence, providing global guidance while remaining unchanged during
                denoising. The reference image(s) can be of any size and will be automatically resized and cropped to
        ...
        for ref_type, reference in [("video", reference_video), ("image", reference_image)]:
            if reference is not None:
                ref_frames = reference  # [F, C, H, W] or [1, C, H, W]
                # extract a zero mask for reference frames latent patches
                ref_num_frames_input = ref_frames.size(0)
                ref_num_frames_output = self.trim_conditioning_sequence(0, ref_num_frames_input, num_frames)
                if filter_reference_video_patches:
                    patch_masks, _ = self.compute_vae_patch_masks_simple(ref_frames[:ref_num_frames_output],
                                                                         black_thresh=1e-3, min_active_frac=0.05, )
                    patch_masks = patch_masks.flatten()
                    # print(f"Reference video patch mask keeps {patch_masks.sum().item()} / {patch_masks.numel()} patches")
                ...
                reference_video_coords = self._scale_video_ids(
                    ...
                    scale_factor=self.vae_spatial_compression_ratio,
                    scale_factor_t=self.vae_temporal_compression_ratio,
                    frame_index=0 if ref_type=="video" else 1000 * self.vae_temporal_compression_ratio,
                    # Reference video starts at frame 0, reference image at a very high frame index
                    device=device,
                )
                ...
                reference_video_coords[:, 0] = reference_video_coords[:, 0] * (1.0 / reference_video_fps)
                # video_coords = video_coords.float()
                # video_coords[:, 0] = video_coords[:, 0] * (1.0 / frame_rate)
                if use_reference_video_as_noise:
                    assert reference_latents.size() == latents.size(), f"Reference latents size {reference_latents.size()} must match latents size {latents.size()} when used as noise"

                    # replace the non conditioned latents with reference latents
                    latents = torch.where(conditioning_mask.unsqueeze(-1) < 1.0, reference_latents, latents)
                    reference_num_latents = 0  # we're not adding any latents, just replacing
                else:
                    # Concatenate reference latents at the beginning: [reference_latents, frame_conditions, target_latents]
                    if filter_reference_video_patches:
                        video_coords = torch.cat([reference_video_coords[:, :, patch_masks], video_coords], dim=2)
                        latents = torch.cat([reference_latents[:, patch_masks, :], latents], dim=1)
                        reference_num_latents = patch_masks.sum().item()
                    else:
                    ...
                    if conditioning_mask is not None:
                        reference_conditioning_mask = torch.ones(
                            (batch_size * num_videos_per_prompt, reference_latents.size(1)), device=device,
                            dtype=torch.float32
                        )
                        ...
                        target_conditioning_mask = torch.zeros(
                            (batch_size * num_videos_per_prompt, latents.size(1) - reference_latents.size(1)),
                            device=device,
                            dtype=torch.float32,
                        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if is_conditioning_image_or_video or reference_video is not None or reference_image is not None:
                ...
                if self.do_classifier_free_guidance:
                    noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)  # each of shape [1, num_latents, C]
        ...
        if reference_video is not None and output_reference_comparison:
            ...
            if reference_video is not None or reference_image is not None:
