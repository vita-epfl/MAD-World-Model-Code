...
class ConditioningConfig(ConfigBaseModel):
    ...
    mode: Literal["none", "reference_video", "reference_image", "pose_to_rgb", "reference_image_video"] = Field(
        default="none",
    ...
    reference_latents_dir: str = Field(
        default="ref_latents",
        description="Directory name for latents of reference videos when using reference_video mode or latents of the reference image when using reference_image or reference_image_video mode.",
    )

    reference_latents_video_dir: str = Field(
        default="no_default_value_found",
        description="Directory name for latents of reference videos when using reference_image_video mode.",
    )

    bbox_latents_dir: str | None = Field(
        default="bbox_latents_24fps_768x512",
        description="Optional directory name for bbox-control latents when using reference_image_video mode.",
    )

    motion_video_conditioning_p: float = Field(
        default=0.1,
        description="Probability of conditioning on the motion video during training",
        ge=0.0,
        le=1.0,
    )
    ...
    latents_dir: str = Field(
        default="latents",
        description="Directory name for latents of input videos",
    )
...
class OptimizationConfig(ConfigBaseModel):
    ...
    #     default=0.9,
    #     description="Beta1 parameter for AdamW optimizer",
    #     ge=0.0,
    #     le=1.0,
    # )
    # optimizer_beta2: float = Field(
    #     default=0.999,
    #     description="Beta2 parameter for AdamW optimizer",
    #     ge=0.0,
    #     le=1.0,
    # )
...
class ValidationConfig(ConfigBaseModel):
    ...
    reference_images: list[str] | None = Field(
        default=None,
        description="List of reference image paths to use for validation. "
                    "One image path must be provided for each validation prompt",
    )
    ...
    fps: int | None = Field(
        default=24,
        description="Frame rate for validation videos. If None, uses the model's default frame rate.",
        gt=0,
    )
    ...
    @classmethod
    def validate_num_reference_images(cls, v: list[str] | None, info: ValidationInfo) -> list[str] | None:
        """Validate that number of reference images (if provided) matches number of prompts."""
        num_prompts = len(info.data.get("prompts", []))
        if v is not None and len(v) != num_prompts:
            raise ValueError(f"Number of reference images ({len(v)}) must match number of prompts ({num_prompts})")
        return v
