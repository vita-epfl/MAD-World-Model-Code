...
class PrecomputedDataset(Dataset):
    ...
    def _get_expected_file_path(self, dir_name: str, data_file: Path, rel_path: Path) -> Path:
        ...
        if dir_name.__contains__("bbox"):
            # For bbox data, replace _rgb with _bbox_0 in relative path
            rel_path = Path(str(rel_path).replace("_rgb", "_bbox_0"))
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        for dir_name, output_key in self.data_sources.items():
            ...
            if str(source_path).__contains__("bbox"):
                # For bbox data, randomly sample the id from 0 to 4 and replace in the relative path
                random_id = random.randint(0, 4)
                file_rel_path = Path(str(file_rel_path).replace("_bbox_0", f"_bbox_{random_id}"))
