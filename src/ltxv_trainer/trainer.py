...
# ---- Torch compiler stance compat ----
def _noop_decorator(*args, **kwargs):
    def _wrap(fn): return fn
    return _wrap

if getattr(torch, "compiler", None) and hasattr(torch.compiler, "set_stance"):
    set_stance = torch.compiler.set_stance
else:
    # PyTorch without compiler.set_stance (e.g., 2.5)
    set_stance = _noop_decorator
# --------------------------------------
...
class LtxvTrainer:
    ...
    def _load_models(self) -> None:
        ...
        components = load_ltxv_components(
            ...
            load_text_encoder_in_8bit=False, #self._config.acceleration.load_text_encoder_in_8bit,
    ...
    def _prepare_models_for_training(self) -> None:
        ...
        # Don’t wrap frozen VAE in DDP, just put it on CPU
        #self._vae.requires_grad_(False).eval().to("cpu")
        self._transformer = prepare(self._transformer)
        self._text_encoder = prepare(self._text_encoder)
        # Do NOT wrap frozen models in DDP
        #self._text_encoder.requires_grad_(False).eval().to("cpu")  # or accelerator.device if you prefer GPU
    ...
    def _init_optimizer(self) -> None:
        ...
        if opt_cfg.optimizer_type == "adamw":
            # beta1 = opt_cfg.optimizer_beta1
            # beta2 = opt_cfg.optimizer_beta2
            # betas = (beta1, beta2)
            optimizer = AdamW(self._trainable_params, lr=lr, betas=(0.9, 0.95))
    ...
    def _create_scheduler(self, optimizer: torch.optim.Optimizer) -> LRScheduler | None:
        ...
        warmup_start_factor = float(params.pop("warmup_start_factor", 0.01))
        ...
        if warmup_steps > 0:
            ...
            if scheduler is None:
                return SequentialLR(optimizer, [warmup], milestones=[warmup_steps])
            return SequentialLR(optimizer, [warmup, scheduler], milestones=[warmup_steps])
        else:
            return scheduler
    ...
    def _setup_accelerator(self) -> None:
        ...
        # Keep the global flag consistent with Accelerate's view
        globals()["IS_MAIN_PROCESS"] = self._accelerator.is_main_process
    ...
    def _sample_videos(self, progress: Progress) -> list[Path] | None:
        ...
        for j, prompt in enumerate(self._config.validation.prompts):
            ...
            fps = self._config.validation.fps
            ...
            if self._config.validation.reference_videos is not None:
                ...
                ref_video, ref_video_fps = read_video(video_path, target_frames=None) #None enable to work with 1 frame but do not crop video longer than expected
                pipeline_inputs["reference_video"] = ref_video
                pipeline_inputs["reference_video_fps"] = ref_video_fps
                pipeline_inputs["use_reference_video_as_noise"] = self._config.conditioning.mode == "pose_to_rgb"

            # Load and add reference image, if provided
            if self._config.validation.reference_images is not None:
                ref_image_path = self._config.validation.reference_images[j]
                ref_image = open_image_as_srgb(ref_image_path)
                if ref_image.size != (width, height):
                    # Resize and center crop the image to match the validation video dimensions
                    ref_image = F.resize(ref_image, size=min(width, height))
                    ref_image = F.center_crop(ref_image, output_size=(height, width))
                # make image of shape (1, C, H, W)
                pipeline_inputs["reference_image"] = F.to_tensor(ref_image).unsqueeze(0)
